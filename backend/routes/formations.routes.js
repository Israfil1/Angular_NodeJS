...
router.get("/", (req, res) => {
  let sql =
    "select * from formations f, formateurs fo, categorie c WHERE f.idFormateur = fo.idFormateur AND f.idCategorie = c.idCategorie";
...
router.get("/:idFormation", (req, res) => {
  ...
  connection.query(sql, (err, result) => {
    if (err) {
      console.log(
        "Erreur lors la récupération de la formation avec id" + idFormation,
        err.message
      );
    }
    if (result.length > 0) {
